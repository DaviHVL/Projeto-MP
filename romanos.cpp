...
bool verificador_vld(const string& num_romano){
    ...
    if (num_romano.length() == 2  && num_romano[0] == num_romano[1] && num_romano[0] == 'V')  {
            return false;
    }

    if (num_romano.length() == 2  && num_romano[0] == num_romano[1] && num_romano[0] == 'L')  {
            return false;
    }

    if (num_romano.length() == 2  && num_romano[0] == num_romano[1] && num_romano[0] == 'D')  {
            return false;
    }
...
int romanos_para_decimal(const string& num_romano){
    ...
    for (size_t i =0; i<(num_romano.length()); i++) {
        if (i+1 < num_romano.length() && mapa_romano[num_romano[i+1]] > mapa_romano[num_romano[i]]){
